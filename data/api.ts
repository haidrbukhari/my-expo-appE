...
export const getMySets = async (): Promise<{ id: string; set: Set; canEdit: boolean }[]> => {
  ...
  return data.map((item: any) => ({ ...item, canEdit: item.set.creator === user })) || null;
};
...
export const getAllLearnCards = async (setid: string) => {
  const response = await fetch(`${API_URL}/cards/learnAll?setid=${setid}`);
  return response.json();
};
